...
class RobotController:
    def __init__(self):
        # Initialize all your robot components here
        self.HSV_LOWER = np.array([40, 80, 80])
        self.HSV_UPPER = np.array([80, 255, 255])
        self.PI_IP = "192.168.58.189"
        self.PI_PORT = 65432
        self.STREAM_URL = 'http://192.168.58.189:8080/?action=stream'
    ...
    def _configure_camera(self):
        ...
        self.cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 312)
        self.cap.set(cv2.CAP_PROP_FPS, 30)

    def send_motor_command(self, action, speed=None):
        try:
    ...
    def speak(self, text):
        if time.time() - self.last_speak_time >= 5:
            try:
...
try:
    ...
    engine.stop()
    # Add all your other tracking functions here...
